...
if __NABOLI_ENV.__NABOLI_FLASH_TP_RUNNING then
    warn("[Naboli] Flash TP is already running; skipping duplicate load.")
    return
end
...
local __NABOLI_OK, __NABOLI_ERR = xpcall(function()
    ...
    -- ==================== CONFIG SYSTEM ====================
    local configFile = "Naboli_FlashTP_Config.json"
    local Config = {
        flashEnabled = true,
        giantEnabled = false,
        ...
        webhookName = "Naboli Hub Notifier",
        position = {
            XScale = 0.5,
    ...
    local function getRarityColor(rarity)
        ...
        if r == "gold" then
            local c = Color3.fromRGB(255, 225, 53)
            return c, (255 * 65536 + 225 * 256 + 53)
    ...
    for _, oldName in ipairs({"Naboli_Hub_Pro", "Naboli_Flash", "V" .. "7BX_Hub_Pro", "V" .. "7BX_Flash"}) do
    ...
    FooterLabel.Text = "Naboli Hub | discord.gg/KFAHeFXsWm"
    ...
    DiscordLabel.Text = "Credits: Naboli"
    ...
    ExecBox.PlaceholderText = "print('Hello Naboli!')"
    ExecBox.Text = ""
    ...
    local function sendWebhook(url, data)
        ...
        if not requestFn then
            warn("[Naboli Notifier] No HTTP request function found in executor!")
            return
        end
        task.spawn(function()
            ...
            if not success then
                warn("[Naboli Notifier] Failed to send webhook request: " .. tostring(response))
            else
                print("[Naboli Notifier] Webhook sent successfully! Status: " .. tostring(response.StatusCode))
            end
        end)
    end
    ...
    WebhookNameBox.Text = Config.webhookName or "Naboli Hub Notifier"
    ...
    ProximityPromptService.PromptButtonHoldBegan:Connect(function(prompt)
        ...
        connection = RunService.PreRender:Connect(function()
            ...
            if not fired and progress >= sliderValue then
                ...
                -- Send Discord Webhook alert if enabled
                if Config.webhookEnabled and Config.webhookUrl and Config.webhookUrl ~= "" then
                    pcall(function()
                                ...
                                local payload = {
                                    ...
                                    embeds = {
                                        {
                                            title = "🌟 Naboli Hub Notifier 🌟",
                                            color = _rarityEmbedNum, -- color based on brainrot rarity
                                            image = {
                                                url = details.Image
                                            },
                                            fields = {
                                        {
                                            name = "👤 Thief Info",
                                            value = "• **Thief:** " .. player.Name .. " (@" .. player.DisplayName .. ")\n• **ID:** " .. tostring(player.UserId),
                                            inline = true
                                        },
                                        {
                                            name = "🎯 Target Base",
                                            value = "• **Stolen From:** " .. details.Owner .. "\n• **Base Plot:** Detected",
                                            inline = true
                                        }
                                    },
                                    footer = {
                                        text = "Naboli Hub | discord.gg/KFAHeFXsWm",
                        ...
                        if details.Category == "Best" then
                            table.insert(payload.embeds[1].fields, {
                                name = "👑 Best Item Stolen",
                                value = "• **" .. details.Name .. "**",
                                inline = false
                            })
                        else
                            table.insert(payload.embeds[1].fields, {
                                name = "🎒 Other Item Stolen",
                                value = "• **" .. details.Name .. "**",
                                inline = false
                            })
                        end
...
if not __NABOLI_OK then
    __NABOLI_ENV.__NABOLI_FLASH_TP_RUNNING = false
    warn("[Naboli] Flash TP load failed: " .. tostring(__NABOLI_ERR))
end
